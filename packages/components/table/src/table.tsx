...
import { TableProps, Col } from "./table-types"
...
export default defineComponent({
	...
	setup(props, ctx) {
		function* flatten(array, depth) {
			...
			for (const item of array) {
				...
				if (item.children && item.children.length && depth > 0) {
					...
					yield* flatten(item.children.map(c=> ({...c, isChild: true})), depth - 1);
				}
			}
		}
		...
		const handleCheckedItemChange = (val) => {
			...
			state.isIndeterminate = checkedCount > 0 && checkedCount < tableHeader.length
	...
	render() {
		const popoverSlot = {
			reference: () => <el-button size="small" circle icon={Setting}></el-button>
		}
		return <div class="v3-table">
					...
					<div>
						<el-popover v-slots={popoverSlot}>
							<div style="display:flex;justify-content: space-between;align-items: center;margin-bottom:5px">
								<el-checkbox size="mini" v-model={this.checkAll} indeterminate={this.isIndeterminate} onChange={this.handleCheckAllChange}>全选</el-checkbox>
								<div>
									<el-button type="text" size="mini" onClick={this.reset}>重置</el-button>
								</div>
							</div>
							<div style="height: 300px">
								<el-scrollbar style="height: 100%">
									<el-checkbox-group v-model={this.checkList} onChange={this.handleCheckedItemChange}>
										<draggable
											...
											animation={300} 
											v-slots={{
												item: ({element})=> <div key={element.prop}><el-checkbox size="mini" label={element.label}></el-checkbox></div>,
											}}
										></draggable>
